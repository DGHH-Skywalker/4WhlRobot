...
LowPassFilter M0_Curr_Flt = LowPassFilter(0.05);
int pwmA = 4;
int pwmB = 2;
int pwmC = 13;
...
float cal_Iq_Id(float current_a,float current_b,float angle_el)
{
  ...
  float I_beta = 0.57735026919f * current_a + 1.15470053838f * current_b;
...
{  
  float I_q_M0_ori=cal_Iq_Id(CS_M0.current_a,CS_M0.current_b,CS_M0.current_c); 
...
void loop() {
  ...
  CS_M0.getPhaseCurrents();//获取三相电流值
  Serial.println(DFOC_M0_Current());
}